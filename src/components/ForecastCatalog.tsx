...
export default function ForecastCatalog({
  ...
  // Собираем топ тегов для тем (исключая сами знаки зодиака и стихии, так как для них есть отдельный фильтр)
  ...
  const tagCounts = allTags.reduce(
    (acc, tag) => {
      if (!excludeFromThemes.includes(tag)) {
        acc[tag] = (acc[tag] || 0) + 1;
      }
      return acc;
    },
  ...
  const filteredPosts = initialPosts.filter((post) => {
    ...
    // 2. Фильтр по Знаку Зодиака (самое умное)
    if (activeSign) {
      const signData = ZODIAC_SIGNS.find((z) => z.id === activeSign);
      if (signData) {
        // Статья подходит, если в ней есть: сам знак, его стихия или тег "общий прогноз"
      ...
      {filteredPosts.length > 0 ? (
        ...
        <div className="text-center py-32 bg-white rounded-3xl border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500">
            По вашему запросу не найдено прогнозов.
          </p>
          <button
            onClick={() => {
              setActiveTag(null);
